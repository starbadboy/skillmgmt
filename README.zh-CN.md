...
- **技能发现** —— 扫描每个代理已知的技能路径（`~/.claude/skills`、`~/.codex/skills`、`~/.cursor/rules`、`~/.gemini/antigravity/skills`，以及厂商中立的 `~/.agents/skills` 等）。自动跟随符号链接目录。
- **漂移检测（Drift）** —— 当同一个技能存在于多个代理但 markdown 内容不一致时自动标记。每次同步、编辑或删除后都会自动刷新漂移状态。
- **技能编辑** —— 按代理切换的 markdown 编辑器，包含 Content / Diff / Deploys 三个标签页。
- **跨代理同步** —— 一键将某个代理的技能内容推送到选定的其他代理。抽屉中的 **Select all / Clear** 按钮可一次性切换所有非来源代理。
- **删除技能** —— 从单个代理中移除技能。
- **Claude Code 插件列表** —— 独立的 Plugins 视图展示已安装的插件、其打包技能数量，并支持一键按插件来源过滤 Skills 视图。
- **批量选择** —— 表头主复选框（支持半选状态），可对所有已安装代理执行批量删除。
...
- **后端** —— Vite dev-server 中间件（`vite.config.ts`）提供一个小型 REST API：
  ...
  - `PUT /api/skills` —— 更新某个代理下技能的名称、摘要和内容
  - `DELETE /api/skills` —— 从某个代理中移除技能
  - `POST /api/skills/sync` —— 将一个代理的技能内容复制到目标代理列表
  - `GET /api/plugins` —— 列出已安装的 Claude Code 插件及其打包技能 / 代理 / 命令 / hook 计数
...
src/
  ...
  main.tsx         # React 根节点
server/
  skillScanner.ts  # 文件系统扫描器 —— 读取每个代理的技能目录（支持符号链接）
  skillStore.ts    # CRUD + 跨代理同步
  pluginScanner.ts # Claude Code 插件发现（installed_plugins.json + manifest 文件）
